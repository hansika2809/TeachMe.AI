...
\begin{description}
    \item[Dataset:] We used the \texttt{arXiv} dataset (version 3.0.0), a standard and widely-used benchmark for text summarization.
    \item[Structure:] The arXiv Dataset is a massive, publicly available collection of over 2.4 million scholarly pre-print articles and metadata across STEM fields including physics, computer science, mathematics, quantitative biology, quantitative finance, and statistics. It is very much close to lecture note summary.
\end{description}
...
\begin{enumerate}
    ...
    \item \textbf{Truncation:}
        \begin{itemize}
            ...
            \item Target summaries (\texttt{highlights}) were truncated to a maximum length of 256 tokens.
...
ROUGE-2 (Overlap of word pairs) & 20.1 \\
ROUGE-L (Longest common subsequence) & 39 \\
BERTScore (Semantic similarity - Precision) & 0.85 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{BERTScore:} The BERTScore precision of 0.85 is a very strong result. This metric measures semantic similarity (plagiarism of meaning rather than just words). A high precision score suggests that the facts and concepts present in the generated summary are highly likely to be found in the reference summary, indicating high factual accuracy and relevance.
\end{itemize}
